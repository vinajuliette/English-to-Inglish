...
class ConverterForm {
  ...
  convert(text) {
    ...
    var englishTextArray = text.split(/([^A-Za-z]+)/g);
    ...
    for (var i in englishTextArray) {
      ...
      // We also strip punctuation and and case.
			let token = englishTextArray[i];
			if (! token.match(/\w+/g)) {
				IPAText.push(token);
				continue;
			}
      var IPAWord = this.tti.lookup(token.toLowerCase().replace(/[^\w\s]|_/g, '').replace(/\s+/g, ' '));
    ...
    return {
      text: IPAText,
      error: currentErrorMessage + ' ' + currentMultiMessage
    };
  }
